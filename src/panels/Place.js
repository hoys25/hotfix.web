...
const Place = ({ item, order, onIncrementPosition, onDecrementPosition, area }) => {
  ...
  return (
    <div className="Place">
      ...
      {(price > 0)
          ? <footer className="Place__footer"><Link to={`/basket/${area.id}/${item.id}`} className="Place__order">Оформить заказ ({price})</Link></footer>
          : <footer className="Place__footer"><a href=" " onClick={e => e.preventDefault()} className="Place__order">Оформить заказ ({price})</a></footer>
      }
    </div>
  );
};
